...
def get_embeddings(word, model, n_defns):
    word_pos, quotes = get_quotes(word) 
    embeddings = model.encode(quotes) #will need to use word_pos when we change model to contextual word embeddings
    _, _, qr_selection = qr(np.array(embeddings).T, pivoting=True)
    qr_selection = qr_selection[0:n_defns]
    embeddings = embeddings[qr_selection]
    return embeddings
...
def select_clusters(words, model, n_defns):
    embeddings = []
    for w in words:
        word_embedding = get_embeddings(w, model, n_defns)
